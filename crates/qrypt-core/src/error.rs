...
impl fmt::Display for QryptError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::DecapsulationFailed => write!(
                f,
                "Decapsulation failed (invalid ciphertext or shared secret verification)"
            ),
            ...
            Self::DecryptionFailure => write!(
                f,
                "Decryption/decoding failure in underlying code/lattice primitive"
            ),
            Self::IncompatibleParameters => write!(
                f,
                "Parameters mismatch between interacting cryptographic primitives"
            ),
            Self::ThresholdExceeded => {
                write!(f, "Error correction threshold exceeded during decoding")
            }
        }
    }
}
