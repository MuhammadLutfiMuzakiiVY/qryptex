...
fn main() {
    ...
    match cli.command {
        ...
        Commands::Encaps {
            ...
            let pk =
                QryptKemHybrid1::deserialize_public_key(&bytes).expect("Deserialization error");
        ...
        Commands::Decaps {
            ...
            let sk =
                QryptKemHybrid1::deserialize_secret_key(&sk_bytes).expect("Deserialization error");
            ...
            let ct =
                QryptKemHybrid1::deserialize_ciphertext(&ct_bytes).expect("Deserialization error");

            let ss = QryptKemHybrid1::decapsulate(&sk, &ct).expect("Decaps failed");
            fs::write(&ss_out, hex::encode(ss.as_ref())).unwrap();

            println!(
                "Decapsulated Shared Secret (HEX): {}",
        ...
        Commands::Sign {
            ...
            let sk_bytes = hex::decode(sk_hex.trim()).expect("Invalid hex in sk file");
            let sk =
                QryptSigHybrid1::deserialize_secret_key(&sk_bytes).expect("Deserialization error");

            let sig = QryptSigHybrid1::sign(&sk, msg.as_bytes(), &mut rng).expect("Sign failed");
            fs::write(
                &sig_out,
                hex::encode(QryptSigHybrid1::serialize_signature(&sig)),
            )
            .unwrap();
        ...
        Commands::Verify {
            ...
            let pk =
                QryptSigHybrid1::deserialize_public_key(&pk_bytes).expect("Deserialization error");
            ...
            let sig =
                QryptSigHybrid1::deserialize_signature(&sig_bytes).expect("Deserialization error");
        ...
        Commands::Inspect => {
            ...
            println!(
                "{:<45} | {:<12} | {:<12} | {:<15}",
                "Algorithm", "PK Size (B)", "SK Size (B)", "CT/Sig Size (B)"
            );
            println!("{:-<45}-+-{:-<12}-+-{:-<12}-+-{:-<15}", "", "", "", "");
            for m in metrics {
                println!(
                    "{:<45} | {:<12} | {:<12} | {:<15}",
                    m.algorithm_name, m.pk_size_bytes, m.sk_size_bytes, m.ct_or_sig_size_bytes
                );
            }
        }
        Commands::Security => {
        ...
        Commands::AuditTiming { samples } => {
            println!(
                "Running Welch's t-test timing audit ({} samples)...",
                samples
            );
        ...
        Commands::Test => {
            ...
            assert_eq!(
                hkem_ss,
                QryptKemHybrid1::decapsulate(&hkem_sk, &hkem_ct).unwrap()
            );
  ...
  "hybrid_kem1": {{
    ...
    "shared_secret_hex": "{}"
            ...
            println!(
                "Deterministic test vectors exported to: {}",
                out_file.display()
            );
        }
        Commands::Version => {
